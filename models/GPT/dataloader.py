...
def createTrainDataset(root_dir='./data', batch_size=32, stride=60, sample_len=125, start=15, end=-15):
    ...
    for file_name in tqdm(file_names):
        ...
        with open(motion_file_path, 'rb') as file:
            ...
            for i in range(start, end1-sample_len, stride):
    ...
    train_dataloader = DataLoader(TrainDataset(smpl_poses, smpl_trans, music_librosa, music_mert), batch_size=batch_size, shuffle=True)
    ...
    for file_name in tqdm(file_names):
        ...
        with open(motion_file_path, 'rb') as file:
            ...
            for i in range(start, end1-sample_len+1, stride):
                smpl_poses.append(smpl_pose[:, i:i+sample_len, :, :])
                smpl_trans.append(smpl_tran[:, i:i+sample_len, :])
                file_names_plus.append(file_name.replace('.pkl', '_{}_{}.pkl'.format(str(i), str(i+sample_len))))
...
from utils.Music_Extraction.prepare_music import make_music_dance_set
def createDemoDataset(root_dir='./data', batch_size=32, stride=120, sample_len=300, start=15, end=-15):
    ...
    make_music_dance_set(music_path, librosa_path)
    file_names = os.listdir(music_path)
    smpl_poses, smpl_trans, music_librosa = [], [], []
    motion_file_path = os.path.join('./I-Dancers/IP/mode1.pkl')
    for file_name in tqdm(file_names):
        with open(motion_file_path, 'rb') as file:
            data = pickle.load(file)
            smpl_pose, smpl_tran = data['smpl_poses'], data['smpl_trans']
        ...
        librosa_file_path = os.path.join(librosa_path, file_name.replace('.mp3', '.pkl'))
        with open(librosa_file_path, 'rb') as file:
            music = pickle.load(file)['music']
            music_librosa.append(music)

    smpl_poses, smpl_trans = np.array(smpl_poses), np.array(smpl_trans)
    print(smpl_poses.shape, smpl_trans.shape)
    print('Demo Dataset len: ', smpl_poses.shape[0])
    demo_dataloader = DataLoader(DemoDataset(smpl_poses, smpl_trans, music_librosa, file_names, root_dir), batch_size=batch_size, shuffle=False)
    return demo_dataloader


class TrainDataset(Dataset):
    def __init__(self, smpl_poses, smpl_trans, music_librosa, music_mert):
    ...
    def __getitem__(self, idx):
        ...
        smpl_root_init = self.smpl_root_init[idx][shuffled_index, :]
        ...
        return {'smpl_trans': smpl_trans, 'smpl_poses': smpl_poses, 'smpl_root_vel': smpl_root_vel, \
                'smpl_root_init': smpl_root_init, 'music_librosa': music_librosa, 'music_mert': music_mert}
...
class DemoDataset(Dataset):
    def __init__(self, smpl_poses, smpl_trans, music_librosa, file_names, root_dir):
        
        music_librosa = music_librosa
        smpl_trans = torch.from_numpy(smpl_trans)
        smpl_root_init, smpl_root_vel = root_preprocess(smpl_trans)
        smpl_poses = torch.from_numpy(smpl_poses)
        smpl_poses = rotation_matrix_to_rotation_6d(smpl_poses)
    ...
    def __getitem__(self, idx):
        return {'smpl_trans': self.smpl_trans[idx], 'smpl_poses': self.smpl_poses[idx],  \
                'smpl_root_vel': self.smpl_root_vel[idx], 'smpl_root_init': self.smpl_root_init[idx], \
                'music_librosa': torch.from_numpy(self.music_librosa[idx]),  'file_name': self.file_names[idx], \
